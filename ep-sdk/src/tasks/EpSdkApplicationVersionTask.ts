...
type TEpSdkApplicationVersionTask_CompareObject = Partial<TEpSdkApplicationVersionTask_Settings> & Required<Pick<ApplicationVersion, "version">>;
...
export interface IEpSdkApplicationVersionTask_GetFuncReturn extends Omit<IEpSdkTask_GetFuncReturn, "epObject"> {
  epObject: ApplicationVersion | undefined;
}
export interface IEpSdkApplicationVersionTask_CreateFuncReturn extends Omit<IEpSdkTask_CreateFuncReturn, "epObject"> {
...
export class EpSdkApplicationVersionTask extends EpSdkVersionTask {
  ...
  protected getEpObjectKeys(epObject: ApplicationVersion | undefined): IEpSdkVersionTask_EpObjectKeys {
    ...
    if(epObject.applicationId === undefined) throw new EpSdkApiContentError(logName, this.constructor.name, 'epObject.applicationId === undefined', {
      epObject: epObject
    });
    return {
   ...
   */
  protected async getFunc(epSdkApplicationVersionTask_Keys: IEpSdkApplicationVersionTask_Keys): Promise<IEpSdkApplicationVersionTask_GetFuncReturn> {
    ...
    const applicationVersion: ApplicationVersion | undefined = await EpSdkApplicationVersionsService.getLatestVersionForApplicationId({
      applicationDomainId: epSdkApplicationVersionTask_Keys.applicationDomainId,
    ...
    EpSdkLogger.trace(EpSdkLogger.createLogEntry(logName, { code: EEpSdkLoggerCodes.TASK_EXECUTE_API_GET, module: this.constructor.name, details: {
      epSdkApplicationVersionTask_Keys: epSdkApplicationVersionTask_Keys,
      applicationVersion: applicationVersion ? applicationVersion : 'undefined'
    }}));

    if(applicationVersion === undefined) return this.Empty_IEpSdkApplicationVersionTask_GetFuncReturn;
  ...
  protected async isUpdateRequiredFunc(epSdkApplicationVersionTask_GetFuncReturn: IEpSdkApplicationVersionTask_GetFuncReturn ): Promise<IEpSdkTask_IsUpdateRequiredFuncReturn> {
    ...
    const existingCompareObject: TEpSdkApplicationVersionTask_CompareObject = {
      ...
      version: epSdkApplicationVersionTask_GetFuncReturn.epObject.version
    };
  ...
  protected async createFunc(): Promise<IEpSdkApplicationVersionTask_CreateFuncReturn> {
    ...
    const applicationVersion: ApplicationVersion = await EpSdkApplicationVersionsService.createApplicationVersion({ 
      applicationDomainId: this.getTaskConfig().applicationDomainId,
      applicationId: this.getTaskConfig().applicationId,
      applicationVersion: create,
      targetLifecycleStateId: this.getTaskConfig().applicationVersionSettings.stateId,
    });
   ...
   */
  protected async updateFunc(epSdkApplicationVersionTask_GetFuncReturn: IEpSdkApplicationVersionTask_GetFuncReturn): Promise<IEpSdkApplicationVersionTask_UpdateFuncReturn> {
    ...
    if(epSdkApplicationVersionTask_GetFuncReturn.epObject === undefined) throw new EpSdkInternalTaskError(logName, this.constructor.name, 'epSdkApplicationVersionTask_GetFuncReturn.epObject === undefined');
    if(epSdkApplicationVersionTask_GetFuncReturn.epObject.id === undefined) throw new EpSdkApiContentError(logName, this.constructor.name, 'epSdkApplicationVersionTask_GetFuncReturn.epObject.id === undefined', {
      epObject: epSdkApplicationVersionTask_GetFuncReturn.epObject
    });
    if(epSdkApplicationVersionTask_GetFuncReturn.epObject.version === undefined) throw new EpSdkApiContentError(logName, this.constructor.name, 'epSdkApplicationVersionTask_GetFuncReturn.epObject.version === undefined', {
      epObject: epSdkApplicationVersionTask_GetFuncReturn.epObject
  ...
  public async execute(): Promise<IEpSdkApplicationVersionTask_ExecuteReturn> { 
    const epSdkTask_ExecuteReturn: IEpSdkApplicationVersionTask_ExecuteReturn = await super.execute();
