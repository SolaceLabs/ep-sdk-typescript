...
const ApplicationName = `${TestConfig.getAppId()}-application-${TestSpecId}`;
const ApplicationShared = true;
...
describe(`${scriptName}`, () => {
    ...
    it(`${scriptName}: should get complete list of latest application versions`, async () => {
      try {
        const epSdkApplicationAndVersionListResponse: EpSdkApplicationAndVersionListResponse = await EpSdkApplicationVersionsService.listLatestVersions({
          applicationDomainIds: ApplicationDomainIdList,
          pageNumber: 1,
          pageSize: 100
        });
        const epSdkApplicationAndVersionList: EpSdkApplicationAndVersionList = epSdkApplicationAndVersionListResponse.data;

        const message = `epSdkApplicationAndVersionList=\n${JSON.stringify(epSdkApplicationAndVersionList, null, 2)}`;        
        expect(epSdkApplicationAndVersionList.length, message).to.equal(NumApplicationDomains);
        expect(epSdkApplicationAndVersionListResponse.meta.pagination.count, message).to.equal(NumApplicationDomains);
        for(const epSdkApplicationAndVersion of epSdkApplicationAndVersionList) {
          expect(epSdkApplicationAndVersion.applicationVersion.version, TestLogger.createApiTestFailMessage('wrong version')).to.equal(ApplicationVersionString_2);
        }
      } catch(e) {
        if(e instanceof ApiError) expect(false, TestLogger.createApiTestFailMessage('failed')).to.be.true;
        expect(e instanceof EpSdkError, TestLogger.createNotEpSdkErrorMessage(e)).to.be.true;
    ...
    it(`${scriptName}: should test paging of latest application versions`, async () => {
      ...
      try {
        while(nextPage !== undefined && nextPage !== null) {
          ...
          const message = `epSdkApplicationAndVersionListResponse=\n${JSON.stringify(epSdkApplicationAndVersionListResponse, null, 2)}`;        
          expect(epSdkApplicationAndVersionList.length, message).to.equal(PageSize);
          expect(epSdkApplicationAndVersionListResponse.meta.pagination.count, message).to.equal(NumApplicationDomains);
          if(nextPage < NumApplicationDomains) {
            expect(epSdkApplicationAndVersionListResponse.meta.pagination.nextPage, message).to.equal(nextPage + 1);
          } else {
            expect(epSdkApplicationAndVersionListResponse.meta.pagination.nextPage, message).to.be.undefined;
          }
          for(const epSdkApplicationAndVersion of epSdkApplicationAndVersionList) {
            expect(epSdkApplicationAndVersion.applicationVersion.version, TestLogger.createApiTestFailMessage('wrong version')).to.equal(ApplicationVersionString_2);
          }
          nextPage = epSdkApplicationAndVersionListResponse.meta.pagination.nextPage;  
        }
      } catch(e) {
        if(e instanceof ApiError) expect(false, TestLogger.createApiTestFailMessage('failed')).to.be.true;
        expect(e instanceof EpSdkError, TestLogger.createNotEpSdkErrorMessage(e)).to.be.true;
    ...
    it(`${scriptName}: should get complete list of latest application versions for any application domain`, async () => {
      try {
        ...
        expect(epSdkApplicationAndVersionList.length, TestLogger.createApiTestFailMessage('no versions found')).to.be.greaterThan(0);
    ...
    it(`${scriptName}: should get the latest version of for all applications`, async () => {
      try {
        ...
        expect(applicationsResponse.data.length, message).to.equal(NumApplicationDomains);
        for(const application of applicationsResponse.data) {
          // get the latest object & version for each application 
          const latest_EpSdkApplicationAndVersionResponse: EpSdkApplicationAndVersionResponse = await EpSdkApplicationVersionsService.getObjectAndVersionForApplicationId({ 
            applicationId: application.id,
            stateId: undefined
          });
          message = `latest_EpSdkApplicationAndVersionResponse=\n${JSON.stringify(latest_EpSdkApplicationAndVersionResponse, null, 2)}`;    
          expect(latest_EpSdkApplicationAndVersionResponse.application.id, message).to.equal(application.id);
          expect(latest_EpSdkApplicationAndVersionResponse.applicationVersion.version, message).to.equal(ApplicationVersionString_2);
          expect(JSON.stringify(latest_EpSdkApplicationAndVersionResponse.meta.versionStringList), message).to.include(ApplicationVersionString_1);    
          expect(JSON.stringify(latest_EpSdkApplicationAndVersionResponse.meta.versionStringList), message).to.include(ApplicationVersionString_2);    
          // get the version 1 for each event object
          const version1_EpSdkApplicationAndVersionResponse: EpSdkApplicationAndVersionResponse = await EpSdkApplicationVersionsService.getObjectAndVersionForApplicationId({ 
            applicationId: application.id,
            stateId: undefined,
            versionString: ApplicationVersionString_1
          });
          message = `version1_EpSdkApplicationAndVersionResponse=\n${JSON.stringify(version1_EpSdkApplicationAndVersionResponse, null, 2)}`;    
          expect(version1_EpSdkApplicationAndVersionResponse.application.id, message).to.equal(application.id);
          expect(version1_EpSdkApplicationAndVersionResponse.applicationVersion.version, message).to.equal(ApplicationVersionString_1);
          expect(JSON.stringify(version1_EpSdkApplicationAndVersionResponse.meta.versionStringList), message).to.include(ApplicationVersionString_1);    
          expect(JSON.stringify(version1_EpSdkApplicationAndVersionResponse.meta.versionStringList), message).to.include(ApplicationVersionString_2);    
        }
      } catch(e) {
        if(e instanceof ApiError) expect(false, TestLogger.createApiTestFailMessage('failed')).to.be.true;
        expect(e instanceof EpSdkError, TestLogger.createNotEpSdkErrorMessage(e)).to.be.true;
